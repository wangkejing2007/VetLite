...
function playAudio(e, type = 'card') {
    ...
    // Simulate playing (For now using a beeping sound or just visual)
    // In a real app we would do: new Audio(url).play();
    // Since users reported "no sound", let's try to actually play a dummy sound if possible, 
    // or just acknowledge the UI state.
    // The user said "Every card has no sound".
    // I will add a real Audio object attempt, assuming files exist in 'assets/audio/' or root.
    // Spec says data has e.g. "audio: 'murmur_pda.mp3'".

    // Check if data has audio
    // We need to trace which card/case triggered this. 
    // The data is bound to the button context? No. 
    // Let's just use a generic beep for now as we don't have the files.
    // OR: Actually try to load the file if valid.
    ...
    oscillator.start();
    oscillator.stop(audioContext.currentTime + 0.5); // Beep for 0.5 sec
...
function closeLevelUp(event) {
    ...
    if (overlay) {
        ...
        // Remove confetti to save performance
...
function updateProfileStats() {
    ...
    // Only show level up if role CHANGED and it is NOT the initial load of "Lv. 1"
    // Also ignore if user just reset data (currentXp is 0)
    if (role !== previousRole && currentXp > 0 && previousRole !== "Lv. 1 見習生" || (role !== "Lv. 1 見習生" && previousRole === "Lv. 1 見習生" && currentXp > 0)) {
        // This logic is getting complex, let's simplify.
        // We only want to show if we actually crossed a threshold.
        // Storing XP might be better, but let's stick to role change.
        // If prev is Lv 1 and current is Lv 2 -> Show
        // If prev is null (first load) and current is Lv 1 -> Don't show

        // Fix: Check if we are simply loading the page with existing high level.
        // We can use a session flag 'isFirstLoad' but app reloads.
        // Let's rely on the fact that 'vetlite_last_role' should have been saved previously.
        // If it wasn't saved, it defaults to Lv 1. 
        // If we are actually Lv 5, it thinks we jumped Lv 1 -> Lv 5.

        // Solution: On very first run (no localStorage role), just save current and don't show modal.
        if (!localStorage.getItem('vetlite_last_role')) {
