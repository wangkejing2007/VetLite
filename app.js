...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Initialize Profile Stats (Sync Header) but suppress Modal on load
...
function loadCase(index) {
    ...
    // Validation for index from localStorage
...
// Profile Logic
// Profile Logic
function updateProfileStats(triggerModal = true) {
    ...
    if (headerBadge) headerBadge.innerText = role;

    // Gamification Check: Level Up
    const storedRole = localStorage.getItem('vetlite_last_role');

    // Always update stored role to current
    if (role !== storedRole) {
        // Only trigger modal if explicitly allowed (not on first load) AND it's a real upgrade
        if (triggerModal && storedRole && currentXp > 0) {
            const overlay = document.getElementById('level-up-overlay');
...
function resetData() {
    if (confirm('確定要清除所有學習紀錄與自訂卡片嗎？\n此動作將重置：\n- 自訂卡片\n- 卡片閱讀進度\n- 臨床病例診斷紀錄\n- 解剖探索進度')) {
        ...
        // Add missing resets for full clear
        localStorage.removeItem('vetlite_last_card_index');
        localStorage.removeItem('vetlite_last_case_index');
        localStorage.removeItem('vetlite_review_list');
        localStorage.removeItem('vetlite_last_role');
