...
function updateProfileStats() {
    ...
    // Update UI Elements regardless of Modal state
    ...
    // Modal Logic
    // If no stored role, this is the very first run (or data cleared).
    // In this case, we just save the current role and DO NOT show the popup.
    if (!storedRole) {
    ...
    // Normal check
    if (role !== storedRole && currentXp > 0) {
        const overlay = document.getElementById('level-up-overlay');
